...
class Game:
    def __init__(self):
        '''Initializes the Qt application, main window, and UI setup.'''
        ...
        try:
            ...
            self.all_players = []
            self.team_players = []
            self.ui.msgDialog = QtWidgets.QDialog(self.MainWindow) #Dialogs Initialization
            ...
            self.ui.menu_Manage_Teams.triggered[QtWidgets.QAction].connect(self.manage) #menu 
            self.ui.BatRadio.toggled.connect(lambda: self.displayList(self.ui.AllPlayersList, self.all_players))#Connecting Signals to Slots
    ...
    def saveTeam(self):
        try:
            ...
            tables = ['Teams', 'batsmen', 'bowlers', 'all_rounder', 'wicket_keeper']
    ...
    def disableWidgets(self):   #Disables required widgets
    ...
    def newOrOpenTeam(self, team_name = None):      #Performs appropriate action upon recieving a team name
        try:
            if team_name == None:
    ...
    def populateList(self, list_name, team_name):
        try:
            if team_name == self.all_players:
                query = ['SELECT player_name, Category FROM Stats;']
            elif team_name == self.team_name:
                query = ['SELECT player_name FROM batsmen, Teams WHERE Teams.Name == batsmen.team_name AND Teams.Name = "'+team_name+'";',
                     'SELECT player_name FROM bowlers, Teams WHERE Teams.Name == bowlers.team_name AND Teams.Name = "'+team_name+'";',
                     'SELECT player_name FROM all_rounder, Teams WHERE Teams.Name == all_rounder.team_name AND Teams.Name = "'+team_name+'";',
                     'SELECT player_name FROM wicket_keeper, Teams WHERE Teams.Name == wicket_keeper.team_name AND Teams.Name = "'+team_name+'";']
    ...
    def getGroup(self, player):
        query = 'SELECT Category FROM Stats Where player_name = ?;'
...
but you cannot add more than {} {}.""".format(group_count, group[1])
    ...
    def checkAddPermissible(self, group, count):
        if (group == 'BAT' and count < 4) or ((group == 'BOWL' or group == 'AR') and count < 3) or (group == 'WK' and count < 1):
            return True
        else:
            return False
...
##SETTINGS FOR EVALUATE DIALOG
    ...
    def getCombos(self):
        try: 
            query = ['SELECT Name FROM Teams', 'SELECT match_number FROM all_games']
            result = self.getQueryResult(query)
